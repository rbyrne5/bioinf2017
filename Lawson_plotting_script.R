...
source("~/R_scripts/FinestructureLibrary.R") # read in the R functions, which also calls the needed packages
source("~/R_scripts/FinestructureDendrogram.R")
...
pdf(file="test_coancestry.pdf",height=12,width=12)
...
tmpmat[tmpmat>tmatmax]<-tmatmax # 
pdf(file="testPopAveragedCoancestry.pdf",height=12,width=12)
plotFinestructure(tmpmat,dimnames(tmpmat)[[1]],dend=tdend,cols=some.colorsEnd,cex.axis=0.6,edgePar=list(p.lwd=0,t.srt=90,t.off=-0.1,t.cex=0.8))
dev.off()
